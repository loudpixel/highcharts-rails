...
class Chart
  ...
  def jquery
    ...
    "var chart_#{@chart[:renderTo]} = new Highcharts.Chart({ 
      #{query_params.join(",\n")} 
    });"
  end

  def fullscreen
    anterior = @chart[:renderTo]
    @chart[:renderTo] = "full_"+@chart[:renderTo]
    @chart[:animation] = false
    salida = jquery
    @chart[:renderTo] = anterior
    
    return salida
    
  end
    
  alias_method :to_s, :jquery
